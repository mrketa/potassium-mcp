...
The recorder, selective reads and exact grouping have source qualification, with a separately scoped real lifecycle-5 idle-recording acceptance described below. The retired optional skill's inert evaluation is historical guidance evidence, not runtime qualification. Candidate npm/Setup builds, reinstall, stable-version decision and publication remain approval-gated. All ownership/workflow-expansion package, EXE, ZIP and soak results below are historical artifact-bound evidence, even when filenames are unchanged. No separate testworld or retired A5/F6/I3 gate is reinstated.
...
### Current isolated test candidate — September 10, 2026
...
The source manifest remains `0.10.0-beta.1`; preserve historical artifact identities without treating them as current candidates. A subsequent `1.0.0` needs deliberate versioning, newly approved package/build checks, documented migration/limits and explicit publication approval; no existing npm version may be overwritten. Source implementation and historical migration proof are not an already released v1.
