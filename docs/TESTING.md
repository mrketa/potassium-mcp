...
The user-approved CI boundary explicitly readies Windows PowerShell/CIM and the built-in security commands before functional qualification. This dependency-readiness step is separately bounded and timed; it changes no runtime/RPC deadline, skips no test, and does not certify cold OS-start latency. Internal helper scripts additionally restrict module discovery to the actual child engine's `$PSHOME/Modules`, including paths reconstructed after process startup. A hosted diagnostic comparison preserved the original CIM/JSON pipeline and five-second cap: the unrestricted arm timed out, while the OS-only arm returned process identity and passed public broker status/stop checks. This diagnostic is not full package qualification. Without timely identity evidence, the runtime continues to fail closed rather than trust a PID.
