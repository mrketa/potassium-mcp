...
Full acceptance (the default receipt scope) covers fresh Install, restricted upgrade, Repair, cancelled removal, confirmed removal, retained reinstall, cold Check, actual installed no-argument launcher, and installed native parser through stock SDK. Only genuinely fresh state gets generic-agent read/admin/execute/global defaults; token, explicit restrictions, wrappers, saved contexts/maps, and ACLs survive maintenance. Recovery must resolve before initial grants. Check must distinguish a healthy MCP connection from an unattached executor.

Interactive startup coverage must prove automatic RunAs occurs before SetupForm, verifies an actual administrator token, and preserves the launching Windows user SID. Exercise approval, cancellation/denial without installation or retry, different-account refusal, and an already-elevated direct launch using its current account. Verify the exact read-only `--verify-bundle` path does not request elevation despite interactive Setup requiring it. The PE manifest remains `asInvoker` intentionally; .NET startup hooks must remain disabled through the existing runtime option. No additional MCP permissions prompt is introduced, but normal Windows UAC must not be described as absent.

Do not confuse OS elevation with MCP policy. Launcher and ParserHost do not require administrator rights, while Setup-launched checks, CLI children, and broker restarts may inherit elevated rights. Record actual integrity where tested instead of claiming all children are medium. Typed ACL guidance in already-elevated Setup must retain original target/native details and ask for target-permission review, without repeated RunAs advice, permission reset, ownership bypass, or automatic retry.
...
The user selected essential manual installer qualification rather than the expanded Windows matrix. An explicit `checks.windows.scope: "essential"` receipt requires `passed`, `fresh`, `repair`, `coldCheck`, `adminStartup` (observed automatic same-user elevation to an actual administrator token), and `normalLauncher` (actual normal-user installed launcher connection) to be true. Its bounded nonempty `userApproval` records that scope decision. Every legacy Windows flag remains boolean; unrun checks stay false, with exactly one bounded nonempty reason per false flag in `checks.windows.limitations`. Keep `clipboardScope` truthful. This does not claim omitted lifecycle, clipboard, native-parser, or additional UAC cases were run; describe narrower observations and additional omissions in the existing top-level limitations. Absent scope or `"full"` remains strict full acceptance, and all non-Windows release gates remain unchanged.
...
Request-context retention checks must exercise repeated stateless HTTP server creation with the stock SDK, not only one retained connection. The module-lifetime async context store must preserve request-specific client selection, cancellation, and overlapping-server isolation without accumulating one AsyncLocalStorage instance per request server. A short diagnostic or focused regression does not supersede a failed earlier candidate soak; qualify the new exact artifact again with the complete unchanged workload and thresholds.
