...
Windows PowerShell 5 helper processes omit inherited `PSModulePath` in every case variant, allowing that engine to rebuild its own module search path instead of loading incompatible PowerShell 7 modules. Installer ACL source/target variables remain explicit; the image helper already uses a restrictive environment whitelist. Broker CIM process inspection has a five-second OS-observation cap to accommodate measured cold startup, still fails closed on missing identity, and does not change RPC, authentication or lifecycle deadlines or cache ownership observations.
