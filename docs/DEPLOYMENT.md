...
Start Potassium once to create its workspace, run Setup, approve normal Windows UAC with the same account, select the existing `%LOCALAPPDATA%\Potassium\workspace` with **Change folder**, and click **Install**. Setup does not install Potassium or Roblox. There is no Desktop/CLI selector, additional MCP permissions prompt, or Safeproxy. Only genuinely fresh state grants generic `agent` full read/admin/execute access and the global execution gate, including synchronous/asynchronous Luau execution. Updates, repairs, and retained reinstalls preserve explicit existing permissions and restrictions. A missing `agent` policy in existing configuration may receive a read-only entry, never elevated MCP grants.
...
Interactive startup requests `RunAs` before opening SetupForm. The relaunched process must prove an actual administrator token and the original Windows user SID; approving with another administrator account is refused to protect per-user roots and private-file ownership. Cancellation or denial stops before installation without automatic retry. If Setup is already launched elevated, it uses that current account; choose the intended account and roots.

The executable manifest deliberately stays `asInvoker`; interactive code requests elevation while exact read-only `--verify-bundle` bypasses that request. Setup disables .NET startup hooks using the existing runtime setting. Launcher and ParserHost do not independently require administrator rights. However, checks, bundled CLI processes, and broker restarts launched by elevated Setup inherit its context; do not assume every child runs at medium integrity. Windows administrator approval and MCP read/admin/execute policy are different controls.
...
Interactive Setup already runs as administrator. For positively identified ACL read/write access or privilege failures, its guidance asks you to review the reported original target path, file ownership, and effective permissions rather than repeatedly reopening it with RunAs. Use the same intended Windows account and installation/workspace roots. Elevation is not a guarantee that every ACL restriction is resolved. PowerShell launch errors such as `EPERM`, missing executables, module initialization failures, malformed output, and other unclassified failures are not proof of an ACL denial.

ACL guidance does not retry operations, reset permissions, or bypass ownership checks; automatic UAC occurs at interactive startup, not as an error-recovery loop. Preserve reported journals/backups and do not delete ownership or credential files to force an upgrade. The details view redacts secrets but intentionally includes the original local target path, so review it before sharing diagnostics.
...
These direct Node CLI commands do not perform Setup's automatic UAC handoff; they inherit the PowerShell session's rights. If the installation's actual ACL requirements call for an elevated terminal, open it through normal Windows approval using the same account. Do not switch accounts, reset ACLs, or blindly retry an uncertain maintenance operation. Review any reported ownership/recovery failure before proceeding.
...
The user-selected essential manual Windows scope is recorded only in `QUALIFICATION.json` under `checks.windows`: `scope: "essential"`, a nonempty `userApproval` (at most 1024 characters), true `adminStartup` and `normalLauncher`, and a `limitations` object whose keys are exactly the false legacy Windows flags, each with a nonempty reason of at most 1024 characters. `passed`, `fresh`, `repair`, and `coldCheck` must still be true; all legacy flags and bounded `clipboardScope` remain present. This records omitted checks honestly, not as passed or as full Windows qualification. The default/`"full"` scope still requires every legacy flag true. Source/artifact binding, Node/npm qualification, preservation, rollback, recovery, maps, soak, unsigned approval, and protected publishing gates are unchanged; selecting essential scope does not authorize publication.
