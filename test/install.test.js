...
import { EXECUTOR_REQUEST_TIMEOUT_MS, install, launcher, MCP_LAUNCHER_TIMEOUT_MS, repair, uninstall } from "../src/install.js";

async function fixture(t) {
  const root = await mkdtemp(path.join(os.tmpdir(), "potassium-install-")); t.after(() => rm(root, { recursive: true, force: true }));
  const installRoot = path.join(root, "MCP"); const workspaceRoot = path.join(root, "workspace"); const mcpConfigPath = path.join(root, ".claude-desktop", "mcp.json"); await Promise.all([mkdir(workspaceRoot, { recursive: true }), mkdir(path.dirname(mcpConfigPath), { recursive: true })]); await writeFile(mcpConfigPath, JSON.stringify({ mcpServers: { existing: { command: "keep" } }, preserved: true }));
  ...
  return { root, installRoot, workspaceRoot, mcpConfigPath, host: "claude-desktop", installPackage, nodeExecutable, run: () => ({ status: 0 }), compileProbe: () => {} };
}
test("launcher derives a testable absolute executable without the test runner path", () => {
  ...
  assert.deepEqual(
    ...
    { type: "stdio", command: nodeExecutable, args: ["C:\\stable\\server.js", "--config", "C:\\stable\\config.json"], timeout: MCP_LAUNCHER_TIMEOUT_MS },
  );
...
test("installer requires an explicit flag for unrestricted Luau execution and generates a workspace-local audit log", async (t) => {
  ...
  assert.equal(repairedConfig.allowUnsafeExecute, false);
...
test("multi-host install, repair, doctor, and selective uninstall share one runtime safely", async (t) => {
  ...
  const options = {
    ...
    cwd: value.root,
    env,
    host: ["claude-code", "vscode", "codex", "manual"],
    runCommand,
  ...
  assert.ok(afterOne.hosts["claude-code"]);
...
test("multi-host collision aborts before mutating shared runtime or other hosts", async (t) => {
  ...
  const desktopPath = path.join(env.APPDATA, "Claude", "claude_desktop_config.json");
  const vscodePath = path.join(env.APPDATA, "Code", "User", "mcp.json");
  await Promise.all([mkdir(path.dirname(desktopPath), { recursive: true }), mkdir(path.dirname(vscodePath), { recursive: true })]);
  await Promise.all([
    writeFile(desktopPath, JSON.stringify({ mcpServers: { existing: { command: "keep" } } })),
    writeFile(vscodePath, JSON.stringify({ servers: { potassium: { command: "foreign" } } })),
  ]);
  await assert.rejects(
    install({
      ...
      cwd: value.root,
      env,
      host: ["claude-desktop", "vscode"],
    }),
    /already exists without proven ownership/,
  );
  assert.equal(await access(value.installRoot).then(() => true).catch(() => false), false);
  assert.deepEqual(JSON.parse(await readFile(desktopPath, "utf8")).mcpServers, { existing: { command: "keep" } });
