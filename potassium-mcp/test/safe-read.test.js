...
test("artifact pages stop at the compact default boundary without lowering explicit read limits", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "potassium-safe-read-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const text = `${"a".repeat(4096)}end`;
  await writeFile(join(directory, "record.txt"), text);
  const request = { root: "audits", path: "record.txt" };
  const first = await readArtifact(request, safeConfig(directory));
  assert.equal(first.text, text.slice(0, 4096));
  assert.equal(first.truncated, true);
  const last = await readArtifact({ ...request, offsetBytes: first.bytesRead }, safeConfig(directory));
  assert.equal(last.text, "end");
  assert.equal(last.truncated, false);
  const complete = await readArtifact({ ...request, maxBytes: 262144 }, safeConfig(directory));
...
test("allows only configured public HTTPS GETs and sanitizes output URLs", async () => {
  ...
  const result = await getAllowedHttps(
    ...
    {
      dnsLookup: publicDns,
      httpsRequest: respondHttps(async (url, options) => {
...
test("rejects unconfigured hosts, non-HTTPS URLs, credentials, and sensitive query names", async () => {
  ...
  const dependencies = { dnsLookup: publicDns, httpsRequest: respondHttps(async () => response("ok")) };
...
test("rejects private DNS results and rejects redirect and binary responses", async () => {
  ...
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/" }, config, { dnsLookup: publicDns, httpsRequest: respondHttps(async () => response("redirect", { status: 302 })) }),
    /status 302/,
  );
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/" }, config, { dnsLookup: publicDns, httpsRequest: respondHttps(async () => response("binary", { contentType: "application/octet-stream" })) }),
    /content type/,
  );
});
...
test("bounds HTTP time and body size before returning response text", async () => {
  const config = safeConfig("unused");
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/", maxBytes: 5 }, config, { dnsLookup: publicDns, httpsRequest: respondHttps(async () => response("ok", { contentLength: 6 })) }),
    /maxBytes/,
  );
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/", maxBytes: 5 }, config, { dnsLookup: publicDns, httpsRequest: respondHttps(async () => response("123456")) }),
    /maxBytes/,
  );
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/", timeoutMs: 1 }, config, {
      dnsLookup: publicDns,
      httpsRequest: respondHttps(async () => new Promise(() => {})),
    }),
    /timed out/,
  );
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/", timeoutMs: 1 }, config, {
      dnsLookup: publicDns,
      httpsRequest: respondHttps(async () => new Response(new ReadableStream({ start() {} }), {
        status: 200,
...
test("uses one deadline for stalled DNS and cancels every rejected HTTP response body", async () => {
  ...
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/", timeoutMs: 5 }, config, {
      ...
      httpsRequest: respondHttps(async () => {
        requestCalls += 1;
  ...
  for (const options of [
    ...
    await assert.rejects(
      () => getAllowedHttps({ url: "https://apis.roblox.com/", maxBytes: 5 }, config, {
        dnsLookup: publicDns,
        httpsRequest: respondHttps(async () => tracked.response),
      }),
    );
    await tracked.cancellation;
  ...
  await assert.rejects(
    () => getAllowedHttps({ url: "https://apis.roblox.com/" }, config, {
      ...
      httpsRequest: respondHttps(async () => new Response(new ReadableStream({
        pull(controller) {
          controller.error(new Error(`private response failure ${token}`));
        },
      }), { headers: { "content-type": "text/plain" } })),
    }),
    (error) => !error.message.includes(token) && /body could not be read/.test(error.message),
  );
});
...
test("trace compact defaults report row and byte coverage rather than silently including more records", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "potassium-trace-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const config = traceConfig(directory);
  await writeFile(join(directory, "rows.ndjson"), `${Array.from({ length: 25 }, (_, at) => JSON.stringify({ kind: "observed", at })).join("\n")}\n`);
  const bounded = await queryTrace({ path: "rows.ndjson" }, config);
  assert.deepEqual(bounded.rows.map((row) => row.at), Array.from({ length: 20 }, (_, index) => index));
  assert.equal(bounded.rowLimitReached, true);
  const complete = await queryTrace({ path: "rows.ndjson", maxRows: 500, maxBytes: 262144 }, config);
  assert.deepEqual(complete.rows.map((row) => row.at), Array.from({ length: 25 }, (_, index) => index));
  assert.equal(complete.rowLimitReached, false);
  const first = `${JSON.stringify({ kind: "first", padding: "x".repeat(4500) })}\n`;
  const second = `${JSON.stringify({ kind: "second", padding: "x".repeat(4500) })}\n`;
  await writeFile(join(directory, "bytes.ndjson"), first + second);
  const byteBounded = await queryTrace({ path: "bytes.ndjson" }, config);
  assert.deepEqual(byteBounded.rows.map((row) => row.kind), ["first"]);
  assert.equal(byteBounded.bytesRead, 8192);
  assert.equal(byteBounded.truncated, true);
  assert.equal(byteBounded.incompleteLine, true);
  const allBytes = await queryTrace({ path: "bytes.ndjson", maxBytes: 262144 }, config);
  assert.deepEqual(allBytes.rows.map((row) => row.kind), ["first", "second"]);
  assert.equal(allBytes.truncated, false);
});

test("trace summaries disclose row data only on explicit opt-in and keep redaction on that path", async (t) => {
  const directory = await mkdtemp(join(tmpdir(), "potassium-trace-"));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const config = traceConfig(directory);
  const detail = "detailed diagnostic payload";
  await writeFile(join(directory, "run.ndjson"), `${JSON.stringify({ kind: "observed", detail, password: token })}\n`);
  const summary = await summarizeTrace({ path: "run.ndjson" }, config);
  assert.equal(Object.hasOwn(summary, "rows"), false);
  assert.equal(JSON.stringify(summary).includes(detail), false);
  const included = await summarizeTrace({ path: "run.ndjson", includeRows: true }, config);
  assert.deepEqual(included.rows, [{ detail, kind: "observed", password: "[REDACTED]" }]);
  assert.equal(JSON.stringify(included).includes(token), false);
  await assert.rejects(() => summarizeTrace({ path: "run.ndjson", includeRows: "true" }, config), /includeRows/);
});
