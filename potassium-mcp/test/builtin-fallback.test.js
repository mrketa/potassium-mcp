...
test("builtin fallback rejects malformed, non-success, and oversized responses without exposing the token", async () => {
  ...
  const result = await clientWith(leaked.fetch).listClients();
...
test("builtin fallback body failures cannot leak credentials or paths after diagnostic dispatch", async () => {
  const privatePath = "C:\\private-fallback-store\\bearer.txt";
  const transport = diagnosticFetch();
  const client = clientWith(async (url, init) => {
    if (JSON.parse(init.body).method !== "tools/call") return transport.fetch(url, init);
    let chunks = 0;
    return new Response(new ReadableStream({
      pull(controller) {
        if (chunks++ === 0) controller.enqueue(new TextEncoder().encode('{"jsonrpc":"2.0",'));
        else controller.error(new Error(`transport failed reading ${privatePath}: Bearer ${token}`));
      },
    }), { headers: { "content-type": "application/json" } });
  });
  await assert.rejects(client.listClients(), (error) => {
    assert.match(error.message, /response body could not be read/);
...
test("builtin fallback preserves timeout classification while a response body stalls", async () => {
  let expire;
  let bodyStarted;
  const started = new Promise((resolve) => { bodyStarted = resolve; });
  const client = clientWith(async () => new Response(new ReadableStream({
    pull() { bodyStarted(); },
  }), { headers: { "content-type": "application/json" } }), {
