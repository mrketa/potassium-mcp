...
An independent opt-in, `--native-editor-token-file`, enables six native desktop editor tools: list/read/open/write/activate/close tabs. They use a separately configured native credential and need no Roblox connection or bootstrap. List/read use read permission; mutations use each caller's existing execute permission plus `allowUnsafeExecute`, not an admin-only or agent-exclusive gate. Enabling or disabling editor tools preserves every authorized agent's existing Luau execution access and independent HTTP grants.

Prefer new draft tabs for generated code. Content is limited to **256 KiB of UTF-8**; replacing a tab requires the prior text's lowercase SHA-256 and is best-effort, not atomic against native UI edits. Dirty tabs cannot be closed. See [editor API](../docs/API.md#native-desktop-editor-tabs) and [local opt-in configuration](../docs/CONFIGURATION.md#native-desktop-editor); safe packaged defaults are unchanged.

## Support freeze and release status

This source targets **1.1.0**, adding optional native desktop editor tools and bounded interaction inventory/jobs. It remains a candidate until exact-artifact qualification and protected publication complete; check the [releases page](https://github.com/mrketa/potassium-mcp/releases) for published assets. Stable **1.0.0** is the historical published baseline. Its original qualification manifests are preserved in the [separate technical archive](https://github.com/mrketa/potassium-mcp/tree/5677385f169c24c87f8879e72bb8f5beee86f0d6/release-evidence/v1.0.0). Windows Setup is unsigned; no verified Authenticode publisher identity is claimed. Archived tests and independently rebuilt binaries do not qualify new release bytes.
...
| Surface | Release qualification target | Evidence boundary |
|---|---|---|
...
| Potassium | 2.4.7, with matching package/deployed/running assets | Current bootstrap build is `lifecycle-6`, including `mapRecording` v1 and interaction inventory/actions v1. Historical lifecycle-4/5 and owned-fixture evidence remains scoped to its exercised builds and operations. Tool discovery or source edits do not prove deployment, native readiness, or universal engine/server semantics. |
...
| Portable host core | Outside this Windows release qualification | Linux host core and its lifecycle cases remain unqualified. No Linux/macOS executor integration, other Node lines or universal adapter support is claimed. |
...
The npm package and Windows Setup EXE/ZIP are the user distributions; GitHub's automatic source archives are not installed runtimes. The manual release workflow verifies and promotes an already built, qualified, pinned 13-file draft set; it does not rebuild npm or Windows payloads. Windows updates are manual and install the selected Setup's embedded bundle. The historical 1.0.0 download list was simplified, and a separate Windows ACL maintenance rebuild did not change the original npm package or tag. Do not replay complete-set promotion against that reduced historical list or restore removed attachments automatically. See [Deployment](../docs/DEPLOYMENT.md) for release and installation contracts.

Historical source, installed-package, Windows maintenance, clipboard, and soak evidence is artifact-bound; it is not a current pass matrix. Native detached/loopback and owned interaction fixtures do not establish real Roblox server serialization, universal native side effects, or every signal mode. The selected release's qualification receipts must identify the exact package, Setup, ZIP, source, and limits. See [Testing](../docs/TESTING.md#qualification-ledger-and-release-acceptance).
...
`potassium_interaction_inventory` adds read-gated summary/rows/detail/release for ClickDetectors, ProximityPrompts and observed TouchTransmitters; no diff or automatic activation. It defaults to Workspace/maxVisited5000, pages20 rows, and retains8 snapshots/512 rows/256KiB each/1MiB total for120seconds. Retained query preserves observation time, original coverage and expiry. Properties and position/distance explicitly retain success/error states; unavailable metadata is never fabricated as zero. Touch row.reference is the transmitter; host.reference is the explicit BasePart action target. Detail can inspect a specifically selected BasePart without claiming exhaustive touch discovery. See [interaction inventory](../docs/API.md#interaction-inventory-v1).

`potassium_interaction_call` uses existing execute/global gates and one async job for one strict click/prompt/boolean-touch native dispatch. It preserves raw sync/async access, all configured native editor tools, every intended local launcher and independent HTTP execute grant; no admin-only or agent-exclusive restriction is added. It captures exact identities, never rebinds queued paths, and has no explicit property/movement controls, automatic begin/end pairs, retries or replay. Required features are interactionInventory1 for reads and interactionActions1/asyncJobs2 for calls.

Touch takes the documented boolean unchanged, not numeric 0/1. Owned-fixture observations do not establish begin/end phases, exactly one event, arbitrary target eligibility, or universal helper side effects. Successful jobs report native dispatch only with `serverAcknowledged: false`, not gameplay success. Native fixture evidence and final installed-artifact qualification are separate; see [API qualification limits](../docs/API.md#native-interaction-qualification-limits).
...
`potassium_map_recording` provides continuous native start/poll(summary only)/mark/stop/save/release, not model-scheduled short windows. `potassium_map_recording_read` separately exposes live poll(frames/events) and offline read(summary/frames/events); these are not lifecycle-tool aliases. Start selects1–4 exact map-bound targets; READY requires a first real frame and an owned running sampler at the native receipt time. A marker is timed at MCP request receipt, not a physical keypress. Recording has a fixed1–60second deadline, explicit misses/partial reasons and120second terminal retention; polling does not sample or renew either deadline. The 1.1.0 lifecycle-6 bootstrap includes mapRecording1; discovering its tools does not prove a matching running sampler.
...
Historical stock-SDK fixture measurements showed that shorter descriptions and compact selected reads can coexist with a larger full catalog. These are workload-specific JSON-byte/call measurements, not exact tokens, universal savings, or installed-artifact qualification. Re-measure the selected release before making a current performance claim; [Testing](../docs/TESTING.md#recorder-immutable-evidence-and-selective-read-checks) defines the evidence boundary.
...
The separate Windows `Setup.exe` enables full read/admin/execute access for its generic `agent` identity on a genuinely fresh installation, including synchronous and asynchronous `execute_luau`. There is no Advanced setup or additional MCP permissions prompt. Interactive Setup automatically requests administrator approval through normal Windows UAC before opening its form; the elevated child must have an actual administrator token and the same Windows user SID. Cancellation, denial, or a different-account credential relaunch stops without installation or automatic retry. An already-elevated direct launch uses its current account. Only connect trusted agents: arbitrary Luau and prompt injection can induce unwanted actions. Existing MCP permissions survive update, repair, and retained reinstall; Windows elevation does not automatically grant other hosts or HTTP execution.
...
The Setup PE manifest intentionally remains `asInvoker`: its interactive startup performs the elevation check itself, while read-only `--verify-bundle` does not request elevation. Setup's .NET startup hooks are disabled using the existing runtime option. The normal Launcher and ParserHost do not require administrator rights, but checks, CLI children, and broker restarts launched from elevated Setup can inherit its rights; they are not promised medium integrity. See [Windows startup and maintenance](../docs/DEPLOYMENT.md#windows-local-installer).
...
Already-dispatched typed interaction jobs follow the same truthful terminal-outcome rule as remote jobs, retaining dispatchStarted/cancellationRequested and zero returned values on successful native dispatch. Cancellation or a host audit/formatting/retention warning must not cause replay.
...
# Enable desktop editor tools independently; preserve all existing permissions
potassium-mcp repair --native-editor-token-file <private-native-token-path>

# Disable editor tools only, not diagnostics or authorized Luau execution
potassium-mcp repair --no-native-editor
...
Windows ACL-preservation failures identify the original affected path and concrete native error, including transaction-journal failures. Management JSON carries structured `acl` details under the originating `MCP_ACL_PRESERVE_FAILED` error, even when a surrounding recovery failure adds context. Because interactive Setup already runs as administrator, its typed access/privilege advice asks the user to review the actual target path and permissions rather than repeatedly relaunch it with RunAs. Unclassified PowerShell/process failures do not justify ACL-specific advice. No ACL reset, ownership bypass, or automatic retry is introduced. See [Windows ACL preservation failures](../docs/DEPLOYMENT.md#windows-acl-preservation-failures).
