...
const BUILTIN_FALLBACK_TIMEOUT_MS = 3000;
...
function validateBuiltinFallbackToken(token) {
...
function redactToken(value, token) {
  ...
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [redactToken(key, token), redactToken(entry, token)]));
  }
  return value;
}
...
class BuiltinFallbackClient {
  ...
  async #request(token, method, params) {
    ...
    let response;
    try {
      // Never allow the fixed diagnostic endpoint to redirect this request.
      try {
