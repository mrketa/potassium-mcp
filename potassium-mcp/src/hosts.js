...
import { createScanner, findNodeAtLocation, parse as parseJsonc, parseTree, SyntaxKind } from "jsonc-parser";
...
/** Keep the host request deadline outside the executor deadline, including user overrides. */
export function launcherTimeout(requestTimeoutMs) {
  return Math.max(40000, requestTimeoutMs + 10000);
}

export function supportsRuntime(metadata) {
  return metadata?.potassiumMcpRuntime?.ownershipSchema === 3
    && metadata?.potassiumMcpRuntime?.launcherProtocol === 1;
}
...
function requireUnambiguousEntry(tree, key) {
  const containers = tree.children.filter((node) => node.children?.[0]?.value === key);
  if (containers.length > 1) throw new Error(`host configuration contains duplicate ${key} properties`);
  const entries = containers[0]?.children?.[1]?.children?.filter((node) => node.children?.[0]?.value === "potassium") ?? [];
  if (entries.length > 1) throw new Error("host configuration contains duplicate potassium entries");
}
...
function insertJsonProperty(text, parent, name, value) {
  ...
  return `${text.slice(0, first.offset)}${property},${text.slice(first.offset)}`;
}
...
function removeJsonProperty(text, parent, property) {
  ...
  const removePreviousComma = index > 0 && index === siblings.length - 1;
  const start = removePreviousComma
    ? siblings[index - 1].offset + siblings[index - 1].length
    : propertyEnd;
  const end = removePreviousComma
    ? property.offset
    : siblings[index + 1]?.offset ?? parent.offset + parent.length - 1;
  ...
  if (comma < 0) return `${text.slice(0, property.offset)}${text.slice(propertyEnd)}`;
  if (removePreviousComma) {
    return `${text.slice(0, comma)}${text.slice(comma + 1, property.offset)}${text.slice(propertyEnd)}`;
  }
  return `${text.slice(0, property.offset)}${text.slice(propertyEnd, comma)}${text.slice(comma + 1)}`;
}

function jsonTransform(target, source, launcher) {
  const { text, tree, value: config } = parseDocument(source);
  requireUnambiguousEntry(tree, target.key);
...
function potassiumTable(source) {
  return /^\s*\[\s*(?:mcp_servers|"mcp_servers"|'mcp_servers')\s*\.\s*(?:potassium|"potassium"|'potassium')\s*\]\s*(?:#.*)?$/m.exec(source);
}

function codexBlockStart(source, block) {
  for (let index = 0; index < source.length;) {
    if ((index === 0 || source[index - 1] === "\n") && source.startsWith(block, index)) return index;
    ...
    const triple = quote === '"' ? '"""' : "'''";
    const delimiter = source.startsWith(triple, index) ? triple : quote;
...
function managedCodexSection(source, launcher) {
  ...
  const blockStart = codexBlockStart(source, block);
...
export function verifyOwnership(hostId, source, launcher, options = {}) {
  ...
  try {
    ...
    const { tree, value: config } = parseDocument(source);
    requireUnambiguousEntry(tree, target.key);
